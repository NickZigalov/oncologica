...
def get_date_range():
    ...
    start_date_obj = current_date - timedelta(days=50)

    # Задайте здесь нужные даты
    #start_date = "2025-08-01"  # Дата начала в формате ГГГГ-ММ-ДД
    #end_date = "2026-02-01"    # Дата окончания в формате ГГГГ-ММ-ДД
...
def get_all_bitrix_contacts_by_id(start_date, end_date):
    ...
    last_id = 0
    batch_count = 0
    max_batches = 500  # Максимальное количество пакетов
    
    print(f"Начинаем обновление контактов из Bitrix24 с {start_date} по {end_date}...")
...
def save_contacts_to_postgres(contacts_data, start_date, end_date):
    ...
    try:
        ...
        INSERT INTO bitrix.raw_contact (
            id, type_id, client_type, source_id, birthdate, date_create, date_modify, date_donor_onco, sex
        ...
        ON CONFLICT (id) DO UPDATE SET
            ...
            date_donor_onco = EXCLUDED.date_donor_onco,
        ...
        for i in range(0, len(contacts_data), batch_size):
            ...
            for contact in batch:
                ...
                date_donor_onco = contact.get('UF_CRM_1759989190637')
                ...
                contact_data = (
                    ...
                    contact.get('TYPE_ID'),
                    contact.get('UF_CRM_1759990941520'),
